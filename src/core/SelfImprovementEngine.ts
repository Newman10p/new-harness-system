...
import { PROJECT_ROOT, AUDIT_LOG_PATH } from "./constants.js";
...
const IMPROVEMENTS_PATH = path.join(MEMORY_DIR, "self-improvements.md");
const BACKUPS_DIR = path.join(PROJECT_ROOT, "state", "backups");
...
export class SelfImprovementEngine {
  ...
  private reflectInterval: number;
  private history: ImprovementPlan[] = [];
  private autoReflectTimer: ReturnType<typeof setTimeout> | null = null;
  private audit: (entry: {
    type: string;
    detail: string;
    ok?: boolean;
   ...
   */
  async triggerReflection(): Promise<{ reflected: boolean; plans: number }> {
   ...
   */
  async reflect(interval?: number): Promise<ImprovementPlan[]> {
   ...
   */
  startAutoReflect(checkIntervalMs: number = 60_000): void {
  ...
  identifyPatterns(log: string): Pattern[] {
    ...
    const failureDescMap = new Map<string, string>();
    ...
    const slowActions = new Map<string, { count: number; maxMs: number }>();
    for (const line of lines) {
      ...
      switch (p.type) {
        ...
        case "policy_violation":
          ...
          actions.push({
            ...
            target: path.join(PROJECT_ROOT, "agent", "identity.md"),
        ...
        case "provider_error":
          actions.push({
            type: "update_config",
            target: path.join(PROJECT_ROOT, "harness.config.json"),
   ...
   */
  async applyImprovement(plan: ImprovementPlan): Promise<void> {
   ...
   */
  private async applyAction(action: ImprovementAction): Promise<{ ok: boolean; error?: string }> {
    try {
   ...
   */
  private async appendToFile(filePath: string, content: string): Promise<{ ok: boolean; error?: string }> {
    try {
   ...
   */
  private async createSkillFile(action: ImprovementAction): Promise<{ ok: boolean; error?: string }> {
    if (!action.content) {
    ...
    try {
      ...
      const skillPath = path.join(skillsDir, action.target);
      await fs.writeFile(skillPath, action.content, "utf-8");
      return { ok: true };
    } catch (err) {
      return { ok: false, error: err instanceof Error ? err.message : String(err) };
    }
  ...
  private async backupFile(filePath: string): Promise<void> {
    try {
      ...
      const backupPath = path.join(BACKUPS_DIR, `${timestamp}--${fileName}`);
  ...
  async persistState(): Promise<void> {
    try {
      ...
      let content = "";
      try {
        content = await fs.readFile(IMPROVEMENTS_PATH, "utf-8");
      } catch {
      ...
      await fs.writeFile(IMPROVEMENTS_PATH, updated, "utf-8");
    } catch {
   ...
   */
  private buildImprovementSection(): string {
  ...
  private async loadState(): Promise<void> {
    try {
      const content = await fs.readFile(IMPROVEMENTS_PATH, "utf-8");
   ...
   */
  private async readAuditLog(n: number): Promise<string> {
    try {
...
let _instance: SelfImprovementEngine | null = null;

export function getSelfImprovementEngine(
  opts?: ConstructorParameters<typeof SelfImprovementEngine>[0]
