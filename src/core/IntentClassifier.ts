...
export interface IntentEntity {
  type: string; // "filepath", "url", "number", "name", "email", etc.
  value: string;
}
...
export class IntentClassifier {
   ...
   */
  classifyType(input: string): { type: IntentType; confidence: number } {
  ...
  private classifyInternal(input: string): IntentResult {
    ...
    // Score each rule
    let bestType: IntentType = "command";
    let bestScore = 0;
   ...
   */
  private detectUrgency(input: string): Urgency {
    const lower = input.toLowerCase();

    // Critical: caps lock, exclamation, critical keywords
    if (
      (input === input.toUpperCase() && input.length > 5) ||
  ...
  private needsClarification(
    ...
    if (input.length < 10 && type !== "casual" && type !== "emergency") return true;

    // Ambiguous corrections
    if (type === "correction" && input.length < 15) return true;
