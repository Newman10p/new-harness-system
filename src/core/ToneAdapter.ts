...
export interface ToneConfig {
  ...
  enthusiasm: number; // 0=flat, 1=enthusiastic
  humor: boolean; // include witty remarks
  proactivity: number; // 0=reactive, 1=proactive
  voiceRate: number; // TTS speech rate multiplier (0.5-2.0)
}
...
export class ToneAdapter {
  private userToneOverrides: Partial<ToneConfig> | null = null;
  ...
  static inferMood(errorCount: number, sessionAgeSec: number, lastUserMessage?: string): UserMood {
    ...
    if (errorCount >= 3) return "frustrated";
    if (errorCount >= 2) return "frustrated";

    // Long session → tired
    if (sessionAgeSec > 3600) return "tired";
  ...
  private adaptToneInternal(situation: SituationContext): ToneConfig {
    const tone: ToneConfig = { ...DEFAULT_TONE, ...this.userToneOverrides };
   ...
   */
  private async loadUserTonePreferences(): Promise<void> {
    try {
...
let _instance: ToneAdapter | null = null;
