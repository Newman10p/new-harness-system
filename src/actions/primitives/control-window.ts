...
async function listWindows(platform: Platform): Promise<unknown[]> {
  if (platform === "darwin") {
...
async function arrangeWindows(platform: Platform, layout: string): Promise<{ layout: string; arranged: number }> {
  const windows = await listWindows(platform);
  if (windows.length === 0) return { layout, arranged: 0 };

  if (platform === "darwin") {
    if (layout === "cascade") {
      const script = windows
        .map((_, i) => `  set position of window 1 of process "${(windows[i] as Record<string, unknown>).app}" to {${i * 30}, ${i * 30}}`)
        .join("\n");
...
${script}
      ...
      const lines = windows.map((w, i) => {
        ...
        return `  set position of window 1 of process "${(w as Record<string, unknown>).app}" to {${col * cw}, ${row * ch}}
  set size of window 1 of process "${(w as Record<string, unknown>).app}" to {${cw}, ${ch}}`;
      });
      await execAsync(`osascript -e '
tell application "System Events"
${lines.join("\n")}
    ...
    await execAsync(
      `powershell -command "[void][Reflection.Assembly]::LoadWithPartialName('System.Windows.Forms'); ([System.Windows.Forms.Screen]::AllScreens | Select-Object -First 1).Bounds"`,
      { timeout: 5_000 }
    ).catch(() => {});
  }
...
async function performWindowOp(
  platform: Platform,
  operation: string,
  opts: { title?: string; app?: string; x?: number; y?: number; width?: number; height?: number }
): Promise<{ operation: string; app?: string; title?: string; detail: string }> {
...
async function windowsWindowOp(
  op: string,
  opts: { title?: string; app?: string; x?: number; y?: number; width?: number; height?: number }
): Promise<{ operation: string; app?: string; title?: string; detail: string }> {
  const appName = opts.app ?? "";
