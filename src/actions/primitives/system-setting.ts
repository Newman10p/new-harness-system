...
export async function systemSetting(
  ...
  try {
    let result: { setting: string; value: string | number | boolean; detail: string };
...
// ─── Volume ────────────────────────────────────────────────────────────
async function setVolume(platform: Platform, level: number, _display?: number) {
  ...
  if (platform === "darwin") {
    ...
    await execAsync(
      ...
      try {
        ...
        await execAsync(
          `powershell -command "Add-Type -TypeDefinition 'using System;using System.Runtime.InteropServices;public class Audio{[DllImport(\"winmm.dll\")]public static extern int waveOutSetVolume(IntPtr h,uint v);}'; [Audio]::waveOutSetVolume([IntPtr]::Zero, ${Math.round(clamped / 100 * 0xFFFF)})"`,
          { timeout: 5_000 }
        );
      }
    });
...
// ─── Brightness ────────────────────────────────────────────────────────
async function setBrightness(platform: Platform, level: number, display?: number) {
  const clamped = Math.max(0, Math.min(100, Math.round(level)));
  ...
  if (platform === "darwin") {
    ...
    try {
      ...
      await execAsync(
        `powershell -command "(Get-WmiObject -Namespace root/WMI -Class WmiMonitorBrightnessMethods).WmiSetBrightness(1, ${clamped})"`,
        { timeout: 10_000 }
      );
    }
  }
...
// ─── WiFi ──────────────────────────────────────────────────────────────
async function setWifi(platform: Platform, enable: boolean) {
  if (platform === "darwin") {
    const device = "Wi-Fi";
    ...
    await execAsync(
      `powershell -command "Get-NetAdapter | Where-Object { $_.InterfaceDescription -match 'Wi-Fi|Wireless|WLAN' } | ${state}-NetAdapter -Confirm:$false"`,
      { timeout: 15_000 }
    );
  }
...
// ─── Bluetooth ─────────────────────────────────────────────────────────
async function setBluetooth(platform: Platform, enable: boolean) {
  if (platform === "darwin") {
    try {
    ...
    await execAsync(
      `powershell -command "Get-Service bthserv | ${enable ? 'Start-Service' : 'Stop-Service'} -Force"`,
      { timeout: 10_000 }
    );
  }
...
// ─── Do Not Disturb ────────────────────────────────────────────────────
async function setDND(platform: Platform, enable: boolean) {
  if (platform === "darwin") {
    ...
    await execAsync(
      `powershell -command "New-BurntToastNotification -Text 'M.A.I. DND' -Sound 'Default' 2>$null; if ($?) { Set-ItemProperty -Path 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Notifications\Settings' -Name 'NOC_GLOBAL_SETTING_TOASTS_ENABLED' -Value ${enable ? 0 : 1 }"`,
      { timeout: 10_000 }
    ).catch(async () => {
...
// ─── Night Shift ───────────────────────────────────────────────────────
async function setNightShift(platform: Platform, enable: boolean) {
  if (platform === "darwin") {
    try {
    ...
    await execAsync(
      `powershell -command "Set-ItemProperty -Path 'HKCU:\Software\Microsoft\Windows\CurrentVersion\CloudStore\Store\DefaultAccount\Current\default$windows.data.bluelightreduction.bluelightreductionstate\windows.data.bluelightreduction.bluelightreductionstate' -Name 'Data' -Value ${enable ? 1 : 0}"`,
      { timeout: 10_000 }
    ).catch(() => {
...
// ─── Resolution ────────────────────────────────────────────────────────
async function setResolution(platform: Platform, resolution: string, display?: number) {
  if (platform === "darwin") {
    const [w, h] = resolution.split("x").map(Number);
    await execAsync(
      `osascript -e 'tell application "System Events" to tell process "System Preferences" to quit'`
    ).catch(() => {});
...
// ─── Sleep ─────────────────────────────────────────────────────────────
async function triggerSleep(platform: Platform) {
  if (platform === "darwin") {
...
// ─── Lock ──────────────────────────────────────────────────────────────
async function triggerLock(platform: Platform) {
  if (platform === "darwin") {
