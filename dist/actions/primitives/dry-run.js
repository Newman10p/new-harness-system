...
async function dryRun(action, ctx) {
    const targetAction = action.targetAction;
    const targetParams = action.targetParams;
    if (!targetAction) {
    ...
    const commandStr = targetParams?.command ?? JSON.stringify(targetParams ?? {});
        ...
        if (typeof analyzeSideEffects === "function") {
            ...
            return {
                ...
                data: {
                    ...
                    sideEffects: report.effects.map((e) => e.description),
