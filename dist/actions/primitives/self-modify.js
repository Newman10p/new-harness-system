...
const VALID_OPERATIONS = ["append", "replace", "insert_before", "insert_after", "remove_section"];
...
async function createBackup(filePath) {
    try {
        const backupDir = node_path_1.default.join(ROOT, "state", "backups");
        ...
        const backupPath = node_path_1.default.join(backupDir, backupName);
...
function findSectionMarker(content, marker) {
...
function findSectionEnd(content, startLine) {
    const lines = content.split("\n");
...
async function selfModify(action, ctx) {
    ...
    const fullPath = node_path_1.default.resolve(ROOT, normalizedTarget);
    try {
        ...
        switch (operation) {
            case "append": {
                const existing = await promises_1.default.readFile(fullPath, "utf-8").catch(() => "");
                result = existing + (existing.endsWith("\n") ? "" : "\n") + content;
                await promises_1.default.writeFile(fullPath, result, "utf-8");
                break;
            }
            case "replace": {
...
//# sourceMappingURL=self-modify.js.map