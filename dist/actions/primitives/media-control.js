...
async function mediaControl(action, ctx) {
...
async function executeMediaCommand(platform, command, app) {
    const targetApp = app ?? "";
    if (platform === "darwin") {
    ...
    if (platform === "linux") {
        ...
        try {
            ...
            try {
                ...
                await execAsync(`dbus-send --print-reply --dest=org.mpris.MediaPlayer2.${targetApp || "player"} ${mprisPath}/org/mpris.MediaPlayer2.Player ${mapDBusMethod(command)}`, { timeout: 5_000 });
    ...
    const appName = targetApp || "Spotify";
    try {
        await execAsync(`powershell -command "(New-Object -ComObject WScript.Shell).SendKeys('${mapWindowsKey(command)}')"`, { timeout: 5_000 });
        return { command, app: appName, detail: `Executed ${command} via global media keys` };
    }
    catch {
        // Try targeted process approach
        await execAsync(`powershell -command "$p = Get-Process '${appName}' -ErrorAction Stop | Select-Object -First 1; $h = $p.MainWindowHandle; if ($h -ne 0) { [Microsoft.VisualBasic.Interaction]::AppActivate('${appName}') }"`, { timeout: 10_000 });
        await execAsync(`powershell -command "(New-Object -ComObject WScript.Shell).SendKeys('${mapWindowsKey(command)}')"`, { timeout: 5_000 });
        return { command, app: appName, detail: `Activated ${appName} and sent ${command}` };
    }
}
// ─── Get current media info ────────────────────────────────────────────
async function getMediaInfo(platform, app) {
    const info = { status: "unknown" };
    if (platform === "darwin") {
...
async function controlMediaVolume(platform, command, app, volume) {
...
// ─── Helpers ───────────────────────────────────────────────────────────
function buildAppleScriptCommand(appName, command) {
...
function mapLinuxCommand(command) {
    const map = {
...
function mapDBusMethod(command) {
    const map = {
...
function mapWindowsKey(command) {
    const map = {
...
async function findActiveMPRIS() {
    try {
        const { stdout } = await execAsync("dbus-send --session --dest=org.freedesktop.DBus --type=method_call --print-reply /org/freedesktop/DBus org.freedesktop.DBus.ListNames 2>/dev/null | rg 'org.mpris.MediaPlayer2'", { timeout: 3_000 });
        const match = stdout.match(/org\.mpris\.MediaPlayer2\.(\w+)/);
        if (match)
            return `/org/mpris/MediaPlayer2/${match[1]}`;
    }
    catch { /* fall through */ }
