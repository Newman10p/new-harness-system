...
exports.screenArrange = screenArrange;
...
async function screenArrange(action, ctx) {
    ...
    const monitors = Array.isArray(action.monitors) ? action.monitors : undefined;
    ...
    try {
        ...
        switch (operation) {
            ...
            case "set-wallpaper":
                if (!wallpaperUrl)
                    return { ok: false, error: 'Missing required "wallpaperUrl" for set-wallpaper.' };
...
// ─── Switch Desktop ───────────────────────────────────────────────────
async function switchDesktop(platform, index, direction) {
    if (platform === "darwin") {
        if (direction === "left") {
            await execAsync(`osascript -e 'tell application "System Events" to key code 123 using {control down, option down}'`, { timeout: 5_000 });
        }
        else if (direction === "right") {
            await execAsync(`osascript -e 'tell application "System Events" to key code 124 using {control down, option down}'`, { timeout: 5_000 });
        }
        else if (index != null) {
    ...
    if (dirKey) {
        await execAsync(`powershell -command "(New-Object -ComObject WScript.Shell).SendKeys('^${dirKey}')"`, { timeout: 5_000 });
    }
    else if (index != null) {
        for (let i = 0; i < index; i++) {
            await execAsync(`powershell -command "(New-Object -ComObject WScript.Shell).SendKeys('^{RIGHT}')"`, { timeout: 5_000 });
        }
    }
    return { operation: "switch-desktop", detail: "Switched virtual desktop" };
}
// ─── Create Desktop ───────────────────────────────────────────────────
async function createDesktop(platform) {
    if (platform === "darwin") {
        await execAsync(`osascript -e 'tell application "System Events" to key code 125 using {control down}'`, { timeout: 5_000 });
...
// ─── Remove Desktop ──────────────────────────────────────────────────
async function removeDesktop(platform, index) {
    if (platform === "darwin") {
    ...
    await execAsync(`powershell -command "(New-Object -ComObject WScript.Shell).SendKeys('^%{F4}')"`, { timeout: 5_000 });
    return { operation: "remove-desktop", detail: `Removed virtual desktop ${index ?? "current"}` };
}
// ─── List Desktops ───────────────────────────────────────────────────
async function listDesktops(platform) {
    const desktops = [];
    if (platform === "darwin") {
...
tell application "System Events"
  ...
  return output
        ...
        try {
            const { stdout } = await execAsync(`powershell -command "@((Get-Process | Where-Object { $_.MainWindowHandle -ne 0 }).Count)"`, { timeout: 5_000 });
...
async function moveToDesktop(platform, app, index) {
    if (!app)
        return { operation: "move-to-desktop", detail: 'Missing "app" to identify window.' };
    if (index == null)
        return { operation: "move-to-desktop", detail: 'Missing "index" for target desktop.' };
    if (platform === "darwin") {
...
tell application "System Events"
  tell process "${app}"
    ...
    // Windows
    await execAsync(`powershell -command "$p = Get-Process '${app}' -ErrorAction SilentlyContinue | Where-Object { $_.MainWindowHandle -ne 0 } | Select-Object -First 1; if ($p) { [Microsoft.VisualBasic.Interaction]::AppActivate('${app}'); Start-Sleep -Milliseconds 200; (New-Object -ComObject WScript.Shell).SendKeys('^+{F${index + 10}}') }"`, { timeout: 10_000 });
    return { operation: "move-to-desktop", detail: `Attempted to move ${app} to desktop ${index}` };
}
// ─── Set Wallpaper ───────────────────────────────────────────────────
async function setWallpaper(platform, url) {
    if (platform === "darwin") {
        await execAsync(`osascript -e 'tell application "System Events" to set picture of current desktop to POSIX file "${url.replace(/"/g, '\\"')}"'`, { timeout: 10_000 });
        return { operation: "set-wallpaper", detail: `Wallpaper set to ${url}` };
    }
    if (platform === "linux") {
        await execAsync(`gsettings set org.gnome.desktop.background picture-uri "file://${url}"`, { timeout: 5_000 });
        return { operation: "set-wallpaper", detail: `Wallpaper set to ${url}` };
    }
    // Windows
    const escapedUrl = url.replace(/'/g, "''");
    await execAsync(`powershell -command "Add-Type -TypeDefinition 'using System;using System.Runtime.InteropServices;public class WP{[DllImport(\\"user32.dll\\",CharSet=CharSet.Auto)]public static extern int SystemParametersInfo(int u,int v,string s,int f);}'; [WP]::SystemParametersInfo(0x0014,0,'${escapedUrl}',3)"`, { timeout: 10_000 });
    return { operation: "set-wallpaper", detail: `Wallpaper set to ${url}` };
}
// ─── Monitor Arrangement ─────────────────────────────────────────────
async function arrangeMonitors(platform, mode, _monitors) {
    if (platform === "darwin") {
    ...
    // Windows
    const { stdout: dispOut } = await execAsync(`powershell -command "Get-CimInstance -ClassName Win32_DesktopMonitor | Select-Object -ExpandProperty Name"`, { timeout: 10_000 });
...
// ─── Helpers ───────────────────────────────────────────────────────────
async function getCurrentLinuxDesktop() {
    try {
...
//# sourceMappingURL=screen-arrange.js.map