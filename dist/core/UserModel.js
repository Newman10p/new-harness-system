...
class UserModel {
    ...
    async save() {
        try {
            ...
            await promises_1.default.writeFile(USER_PROFILE_PATH, content, "utf-8");
            this.dirty = false;
        }
        catch {
     ...
     */
    async mergePreference(newPref) {
        const existing = this.profile.detectedPreferences.find((p) => p.category === newPref.category && p.key === newPref.key);
        if (existing) {
    ...
    detectBehaviors(params) {
        ...
        if (lower.includes("test") && params.actions.includes("execute-terminal") && params.success) {
            this.observeBehavior("User frequently runs tests via terminal", "Pre-emptively offer to run tests after code changes");
        }
        ...
        if ((lower.includes("what") || lower.includes("show") || lower.includes("list")) &&
            ...
            this.observeBehavior("User often inspects before acting", "Provide file/state context proactively before making changes");
        }
        // User asks for status/summary
        if (lower.includes("status") || lower.includes("summary") || lower.includes("what's going on")) {
            this.observeBehavior("User requests status updates regularly", "Offer periodic status digests");
        }
    }
    observeBehavior(pattern, autoAction) {
    ...
    updateCommonErrors(errors) {
        for (const err of errors) {
            const existing = this.profile.interactionStats.commonErrors.find((e) => e.toLowerCase().includes(err.toLowerCase().slice(0, 30)));
    ...
    parseProfile(content) {
        ...
        try {
            ...
            if (fmMatch) {
                ...
                if (tsMatch)
                    profile.lastUpdated = tsMatch[1].trim();
                const sessMatch = yaml.match(/totalSessions:\s*(\d+)/);
                if (sessMatch)
                    profile.interactionStats.totalSessions = parseInt(sessMatch[1], 10);
                const srMatch = yaml.match(/successRate:\s*([\d.]+)/);
                if (srMatch)
                    profile.interactionStats.successRate = parseFloat(srMatch[1]);
                const aliMatch = yaml.match(/avgLoopIterations:\s*([\d.]+)/);
                if (aliMatch)
                    profile.interactionStats.avgLoopIterations = parseFloat(aliMatch[1]);
            }
            // Extract preferences from markdown
            const prefRegex = /- \*\*(\w+)\/(\w+)\*\*:\s*(.+?)\s*\((\d+)% confidence\)/g;
            let match;
