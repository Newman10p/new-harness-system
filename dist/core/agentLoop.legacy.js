 ...
 */
async function startAgentLoop(orchestrator, agentState, workflowEngine, eventBus = eventBus_1.globalEventBus) {
        ...
        switch (event.type) {
            case "user_input": {
                ...
                if (text.startsWith("pause ")) {
                    ...
                    if (wf) {
                        workflowEngine.pause(wf.id);
                        console.log(`  ⏸ Paused: ${wf.name}`);
                    }
                    return;
                }
                if (text.startsWith("resume ")) {
                    ...
                    const wf = workflowEngine.list().find((w) => w.name.toLowerCase().includes(target));
                    if (wf) {
                        workflowEngine.resume(wf.id);
                        console.log(`  ▶ Resumed: ${wf.name}`);
                    }
                    return;
                }
                if (text.startsWith("cancel ")) {
                    ...
                    const wf = workflowEngine.list().find((w) => w.name.toLowerCase().includes(target));
                    if (wf) {
                        workflowEngine.cancel(wf.id);
                        console.log(`  ⛔ Cancelled: ${wf.name}`);
                    }
                    return;
                }
...
//# sourceMappingURL=agentLoop.legacy.js.map