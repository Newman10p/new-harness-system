...
class SessionManager {
    ...
    constructor(config, logFn) {
        const defaults = {
            sessionTtlMs: types_js_1.DEFAULT_AUTH_CONFIG.sessionTtlMs,
            tokenTtlMs: types_js_1.DEFAULT_AUTH_CONFIG.tokenTtlMs,
            maxSessionsPerUser: types_js_1.DEFAULT_AUTH_CONFIG.maxSessionsPerUser,
        };
    ...
    */
    createSession(userId, deviceId, channel, opts) {
    ...
    */
    createToken(userId, deviceId, scope) {
        const now = Date.now();
        const token = {
    ...
    */
    ...
    getActiveSessions() {
        const now = Date.now();
        return Array.from(this.sessions.values()).filter((s) => s.valid && s.expiresAt > now);
    }
    /** Get active sessions for a specific user. */
    getUserSessions(userId) {
        const now = Date.now();
        return Array.from(this.sessions.values()).filter((s) => s.userId === userId && s.valid && s.expiresAt > now);
    }
    ...
    countUserSessions(userId) {
        const now = Date.now();
        return Array.from(this.sessions.values()).filter((s) => s.userId === userId && s.valid && s.expiresAt > now).length;
    }
    /** Count active tokens. */
    countActiveTokens() {
        const now = Date.now();
